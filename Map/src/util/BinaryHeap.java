    ...
    */
    ...
    public void ensureCapacity(){
        if(array.length <= size){
            ...
            array = Arrays.copyOf(array, newsize); //it copies the specified array, truncating or padding with false (if necessary) so the copy has the specified length. 
        }
    }
    ...
    public void bubbleUp(int i){
        while (i > 1) {
            if(comparator.compare((E)array[i], (E)array[i/2]) >= 0) break;
            Object obj = array[i];
            array[i] = array[i/2];
            array[i/2] = obj;
            i /= 2;
